...
func_tran:固定带宽
func_repl:函数置换策略(0:LRU|1:LFU|2:FIFO)
task_sort:任务排序策略
'''
def TFHO(func_n, func_cache, func_deal_media, cloud_hop, _lambda, func_tran, func_repl, task_sort, node_select):
    # 1.读取三个数据集中的数据
        ...
        if t < 1000:
            ...
            for i in np.arange(n):
                ...
                while j < req_num:
                    ...
                    '''------[边缘节点卸载决策]------
                    1.存在函数&&处理能力够->本地直接处理
                    2.本地处理能力不足->卸载到其它边缘或者云中心
                    3.不存在函数&&处理能力够->卸载到其它边缘或者云中心+拉取函数冷启动
                    '''
                    # ---情况1---
                    ...
                    # ---情况2---
                        ...
                        select_n = -1
                        '''------[节点选择策略]------'''
                        # 0:最短路径+存在函数+处理能力够|1:负载最低+存在函数+处理能力够
                        if node_select == 0:
                            # 索引0处是该边缘节点本身
                            for nd in np.arange(1, len(hops[i])):
                                if (func_each[hops[i][nd]].inCache(select_func)) and (func_deal[hops[i][nd]] >= result[index][0]):
                                    select_n = hops[i][nd]
                                    break
                        else:
                            # 寻找当前负载最低的节点
                            ...
                            for nd in np.arange(n):
                                if (func_each[nd].inCache(select_func)) and (func_deal[nd] >= result[index][0]) and (func_deal[nd] > min_deal):
                                    select_n = nd
                                    min_deal = func_deal[nd]
                        # 没有边缘节点可以处理,交由云计算中心
                        if select_n == -1:
                            complete[result[index][4]] = math.ceil(result[index][1] / func_tran) * cloud_hop
                        else:
                            tran_list[select_n].append([result[index], math.ceil(result[index][1] / func_tran) * get_count_by_dijkstra(G, i, select_n), select_func])
                    # ---情况3---
                    else: 
                        # 尝试卸载到其它边缘节点
                        select_n = -1
                        ...
                        if select_n == -1:
                            ...
                            tmp = math.ceil(result[index][1] / func_tran) * get_count_by_dijkstra(G, i, select_n)
                            # 卸载到其它边缘节点更优
                            if tmp < func[select_func][2]:
                                tran_list[select_n].append([result[index], tmp, select_func])
                            # 冷启动更优
                            else:
                                wait_list[i].append([result[index], select_func])
                    j += 1
...
TFHO(func_n, func_cache, func_deal_media, cloud_hop, _lambda, func_tran, func_repl, task_sort, node_select)
'''

# print(TFHO(20, 5, 200, 10, 4, 0.1, 0, 1, 0))
# print(TFHO(20, 5, 200, 10, 4, 0.1, 1, 1, 0))
# print(TFHO(20, 5, 200, 10, 4, 0.1, 2, 1, 0))

# print(TFHO(40, 5, 200, 10, 8, 0.1, 0, 1, 1))
# print(TFHO(40, 5, 200, 10, 8, 0.1, 1, 1, 1))
# print(TFHO(40, 5, 200, 10, 8, 0.1, 2, 1, 1))
...
# node_select_str = "short-path"  #0
# node_select_str = "low-load"  #1
            ...
            for ch in range(10, 25, 5):#10 15 20
                for lm in range(4, 14, 2):#4 6 8 10 12
                    for ft in np.arange(0.1, 0.25, 0.05):#0.1 0.15 0.2
                        tss = 7
                        nss = 1
                        t1 = TFHO(fn, fc, fdm, ch, lm, ft, 2, tss, nss)
                        t2 = TFHO(fn, fc, fdm, ch, lm, ft, 2, tss, nss)
                        t3 = TFHO(fn, fc, fdm, ch, lm, ft, 2, tss, nss)
                        if tss == 1:
                            task_sort_str = "time-bg"
                        elif tss == 3:
                        ...
                        if nss == 0:
                            node_select_str = "short-path"
                        elif nss == 1:
                            node_select_str = "low-load"
                        data.loc[len(data)] = [fn, fc, fdm, ch, lm, ft, func_repl_str, task_sort_str, node_select_str, 'local_cloud_edge', (t1 + t2 + t3) / 3]

data.to_csv("data/1-71.csv")