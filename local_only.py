            ...
            for j in np.arange(len(wait_list[i])):
                ...
                if func_deal[i] >= wait_list[i][j][0][0]:
                    ...
                    if func_each[i].inCache(tmp):
                        func_each[i].get(tmp)
                        complete[wait_list[i][j][0][4]] = t - time[wait_list[i][j][0][4]]
                    else:
                        func_each[i].put(tmp, tmp)
                        wait_list[i][j][0][2] += func[wait_list[i][j][1]][2]
                        complete[wait_list[i][j][0][4]] = t - time[wait_list[i][j][0][4]] + func[wait_list[i][j][1]][2]
                    # 加入执行队列
            ...
            for ch in range(10, 25, 5):#10 15 20
                for lm in range(4, 14, 2):#4 6 8 10 12
                    for ft in np.arange(0.1, 0.25, 0.05):#0.1 0.15 0.2
                        ...
                        data.loc[len(data)] = [fn, fc, fdm, ch, lm, ft, func_repl_str, task_sort_str, "none", 'local_only', (t1 + t2 + t3 + t4 + t5) / 5]

data.to_csv("data/3-7.csv")